...
for (let i = 0; i < needToEnterCars; i++) {
    ...
    listOfCars[i].FirmName = prompt(`Car left: ${carLeftToEnter} \nEnter brand of car:`);
    ...
    listOfCars[i].ModelName = prompt(`Car left: ${carLeftToEnter} \nEnter model of car:`);
...
for (let i = 0; i < listOfCars.length; i++) {
    if (listOfCars[i].EngineDisplacement >= engineDisplacementLimit) {
        console.log(`${listOfCars[i].FirmName} ${listOfCars[i].ModelName} has engine ${listOfCars[i].EngineDisplacement} l. it's more than or equal ${engineDisplacementLimit} l.`);
        carsWithBigEngine++;
    }
}
